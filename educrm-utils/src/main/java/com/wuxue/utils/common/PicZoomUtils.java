...
//}
    ...
    private static String DEFAULT_PREVFIX = ".thumb.jpg";
    ...
    public void thumbnailImage(File imgFile, int w, int h, String prevfix, boolean force){
        if(imgFile.exists()){
            try {
                ...
                String p = imgFile.getPath();
                // 将图片保存在原目录并加上前缀
                ImageIO.write(bi, suffix, new File(p.substring(0,p.lastIndexOf(File.separator)) + File.separator +imgFile.getName() + prevfix));
            } catch (IOException e) {
                log.error("generate thumbnail image failed.",e);
            }
        }else{
    ...
    public static void main(String[] args) {
        new PicZoomUtils().thumbnailImage("E:/home/data/tomcat/webapps/files/yuyue.jpg", 100, 150);
    }

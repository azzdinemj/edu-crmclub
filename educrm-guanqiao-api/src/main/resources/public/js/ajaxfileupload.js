...
jQuery('<input type="hidden" name="'+i+'" value="' + data[i] + '" />').appendTo(form);
				...
				jQuery(form).attr('action', this.config.url);
				jQuery(form).appendTo('body');

				return form;
			},

			createUploadFormByIdArray : function(id, fileElementIdArr, data) {
				// create form
				var formId = 'jUploadForm' + id;
				var fileId = 'jUploadFile' + id;
				var form = jQuery('<form  action="" method="POST" name="'
				...
				//文件
				for ( var i = 0, size = fileElementIdArr.length; i < size; i++) {
					var tempId = fileElementIdArr[i];
					var oldElement = jQuery('#' + tempId);
					jQuery(oldElement).attr('name', "files");
			...
			ajaxFileUploadByArrayId : function(s) {
				// TODO introduce global settings, allowing the client to modify
				// them for all requests, not only timeout
				var config = {};
				config.url = s.url;
				...
				s = jQuery.extend({}, jQuery.ajaxSettings, s);
				var id = new Date().getTime();
				var form = $.createUploadFormByIdArray(id,
						s.fileElementIdArr, s.data);
...
// console.log("s.url="+s.url);     
					var form = jQuery('#' + formId);
					jQuery(form).attr('action', config.url);
					jQuery(form).attr('method', 'POST');
					jQuery(form).attr('target', frameId);
					if (form.encoding) {
...
// console.log("form.html="+form.html());                    
			...
			handleError : function(s, xhr, status, e) {
				...
				// Fire the global callback
				if (s.global) {
					(s.context ? $(s.context) : $.event).trigger("ajaxError", [
							xhr, s, e ]);
				}
			},

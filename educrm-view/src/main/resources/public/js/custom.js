        ...
        jQuery('#table').dataTable({
            ...
            "drawCallback": function () {
                if($('.selectpicker').length){
                    $('.selectpicker').selectpicker('refresh');//刷新 .selectpicker 插件
                }
            }
        })
    }
...
$(function(){
    ...
    $('#chkAll2').click(function()	{
        if($(this).prop('checked'))	{
            $('.px2').prop('checked',true);
        }
        else	{
            $('.px2').prop('checked',false);
        }
    });
    $('#chkAll3').click(function()	{
        if($(this).prop('checked'))	{
            $('.px3').prop('checked',true);
        }
        else	{
            $('.px3').prop('checked',false);
        }
    });
    /*角色管理里角色的选取*/
    /*全选*/
    $('.out-all').click(function()	{
        if($(this).prop('checked'))	{
            $(this).parent().parent().find('.out-all').prop('checked',true);
            //$('.out-all').parent().parent().parent().addClass('active');
        }
        else	{
            $(this).parent().parent().find('.out-all').prop('checked',false);
            //$('.out-all').parent().parent().parent().removeClass('active');
        }
    });

    $('.out-all2').click(function()	{
        if($(this).prop('checked'))	{
            $(this).parent().parent().find('.out-all2').prop('checked',true);
            //$('.out-all').parent().parent().parent().addClass('active');
        }
        else	{
            $(this).parent().parent().find('.out-all2').prop('checked',false);
            //$('.out-all').parent().parent().parent().removeClass('active');
        }
    });
...
$(document).ready(function() {
    ...
    $('#datas2').daterangepicker(null, function(start, end, label) {
        console.log(start.toISOString(), end.toISOString(), label);
    });
    $('#datas3').daterangepicker(null, function(start, end, label) {
        console.log(start.toISOString(), end.toISOString(), label);
    });
    $('#datas4').daterangepicker({
        "timePicker": false,
...
$(function() {
    $('.table-striped').on('click', 'tr', function (e) {
        // 获取table中所有的行
        var trs = $(this).siblings('tr');
        $.each(trs, function (index, item) {
            $(item).find('td:eq(0) input').prop('checked', false);
            $(item).removeClass('table-color-');
            if ($(item).hasClass("odd")) {
                $(item).find("td").css("background-color", "#f5f5f5");
            }
        });
        // 选中当前点击的行
        //alert($(this).find('td:eq(0) input').prop('checked'));
        if ($(this).hasClass("table-color-")) {
            ...
            if ($(this).hasClass("odd")) {
                $(this).find("td").css("background-color", "#00539c");
            }
        }
    ...
    $('.table1-striped').on('click', 'tr', function (e) {
        // 选中当前点击的行
        //alert($(this).find('td:eq(0) input').prop('checked'));
        if ($(this).hasClass("table-color-")) {
            $(this).find('td:eq(0) input').prop('checked', false);

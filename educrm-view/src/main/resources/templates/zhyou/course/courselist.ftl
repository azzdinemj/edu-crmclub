...
<div class="contentpanel">
    <div class="pageheader">
        <h2><i class="fa fa-bookmark"></i>课程列表<span>...</span></h2>
                    ...
                    <#--<#if course.list??>-->
                        ...
                        <#--<tr>-->
                            ...
                            <#--<td>-->
                                ...
                                <#--<a onclick="delCourse('${(v.pkCourse)!}')" class="btn btn-danger btn-sm">删除</a>-->
...
//                    //+'<a onclick="delCourse('+row.pkCourse.toString()+')" class="btn btn-danger btn-sm">'+"删除"+'</a>'
//        }}
//    ];
    ...
    function renderOperate(data, type, row) {
        return '<a href="/course/courseList/courseware?pkCourse='+row.pkCourse+'&flag=${(flag)!}" class="btn btn-newblue btn-sm">'+"课件"+'</a> '
    }

    function renderIsissue(data) {
        if (data==0){ return '未申请';}
        else if(data==1){return '发布申请'; }
        else if(data==2){return '已审核'; }
    }

    function query() {
        var data  = {"caption":$("#captionCourse").val(),"isissue":$("#isissueCourse").val(),"flag":$("#flag").val()};
        //init(columns,"/course/courseList/queryByPaging",data,"pagingTable");

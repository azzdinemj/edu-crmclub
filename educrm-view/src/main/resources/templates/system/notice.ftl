        ...
        <div class="modal-body">
            <form class="form-ajax form-horizontal look" method="post" action="/system/notice/save"
                ...
                <div class="form-group">
                        <label class="control-label col-xs-2 col-md-2">状态</label>
                        <div class=" col-xs-10 col-md-10">
                            <select name="isdel">
                                <option <#if notice??&&notice.isdel??&&notice.isdel==1> selected="selected" </#if> value="1">启用</option>
                                <option <#if notice??&&notice.isdel??&&notice.isdel==0> selected="selected" </#if> value="0">隐藏</option>
                            </select>
                        </div><!-- /.col -->
                </div>
                    <div class="form-group">
                        <label class="control-label col-xs-2 col-md-2">通知人群</label>
                        <div class=" col-xs-10 col-md-10">
                            <select name="type">
                                <option <#if notice??&&notice.type??&&notice.type==2> selected="selected" </#if> value="2">全部</option>
                                <option <#if notice??&&notice.type??&&notice.type==0> selected="selected" </#if> value="0">教师</option>
                                <option <#if notice??&&notice.type??&&notice.type==1> selected="selected" </#if> value="1">学生</option>
                            </select>
                        </div><!-- /.col -->
                    </div>

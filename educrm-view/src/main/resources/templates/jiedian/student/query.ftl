    ...
    <div class="pageheader">
        ...
        <div class="breadcrumb-wrapper">
            <div class="btn-group fr title-btn">
                <a class="btn btn-sm btn-newblue" href="/jiedian/student/student/create">新建</a>
...
<script type="text/javascript">
    ...
    function renderCaption(data, type, row) {
        if (row.istype == 0) {
            return '<a href="/jiedian/student/student/edit?pkStudent=' + row.pkStudent + '" class="yellow" >' + data + '</a>';
        } else {
            return '<a href="/jiedian/student/student/edit?pkStudent=' + row.pkStudent + '" class="yellow" >' + data + '</a>';
        }
    }
    function query() {
        ...
        init(GetTableColumn("student"), "/jiedian/student/student/queryByPaging", data,"pagingTable");
    }
    function del() {
...
//        alert(getTableSelectRow(3).pkStudent);
    ...
    function stuSchedule() {
        ...
        var index = $(".table-color-").index();
        var id = getTableSelectRow("pagingTable",index).pkStudent;
//        var studentName = getTableSelectRow("pagingTable",index).caption;

        if (id == null) {
            Notify.danger("请先选择要排课的学员");
            return;
        }

        window.location.href = "/jiedian/schedule/query?pkStudent=" + id;

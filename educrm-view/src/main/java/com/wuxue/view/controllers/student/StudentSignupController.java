 ...
 */
@Controller
@RequestMapping(value = "/student/studentSignup")
...
//        String servletPath = request.getServletPath();
        String servletPath = "/student/studentSignup/edit";
...
//        model.addAttribute("capid",capid);
        ...
        if(employeekey!= null && employeekey.equals(JobPostEnum.TEACHER_EMPLOYEE)){
            model.addAttribute("pkJobPost", JobPostEnum.TEACHER_EMPLOYEE);//老师
        }else if (employeekey!= null && employeekey.equals(JobPostEnum.MARKET_EMPLOYEE)){
            model.addAttribute("pkJobPost", JobPostEnum.MARKET_EMPLOYEE);//招生老师
        }else if (employeekey!= null && employeekey.equals(JobPostEnum.INTERVIEW_EMPLOYEE)){
            model.addAttribute("pkJobPost", JobPostEnum.INTERVIEW_EMPLOYEE);//面试老师（教学主管）
        }else if (employeekey!= null && employeekey.equals(JobPostEnum.FINANCIAL_EMPLOYEE)){
            model.addAttribute("pkJobPost", JobPostEnum.FINANCIAL_EMPLOYEE);//财务
        }else if (employeekey!= null && employeekey.equals(JobPostEnum.HEAD_TEACHER_EMPLOYEE)){
            model.addAttribute("pkJobPost", JobPostEnum.HEAD_TEACHER_EMPLOYEE);//班主任
        }else if (employeekey!= null && employeekey.equals(JobPostEnum.SECOND_TEACHER_EMPLOYEE)){
            model.addAttribute("pkJobPost", JobPostEnum.SECOND_TEACHER_EMPLOYEE);//副班主任
        }else if (employeekey!= null && employeekey.equals(JobPostEnum.DIRECTOR_EMPLOYEE)){
            model.addAttribute("pkJobPost", JobPostEnum.DIRECTOR_EMPLOYEE);//学部主任
        }
        if (employeekey!= null&& !"".equals(employeekey)){
            model.addAttribute("pkJobPost",employeekey);
        }
    ...
    public Map<String, Object> queryByPaging(HttpServletRequest request, HttpServletResponse response, Student student, Integer sEcho, Integer start, Integer length) {
        ...
        if(student.getPageSize()==null){
            ...
            student.setPageNo((start/student.getPageSize())+1);
        }

        if(student.getIstype() != null && student.getIstype()==3){
    ...
    public Response getStudentId(HttpServletRequest request,String pkClassinfo) {

        Response response = studentClient.getStudentIdCode(pkClassinfo);

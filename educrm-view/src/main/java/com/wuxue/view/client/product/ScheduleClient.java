...
//        Response<Object> response = JSON.parseObject(responseXml, new TypeReference<Response<Object>>(){});
    ...
    public Response<List<Schedule>> findElecSchedule(Schedule schedule) {
        String responseXml = POST(getSendUrl(ActionEnum.FINDELECSCHEDULE), schedule);
        Response<List<Schedule>> response = JSON.parseObject(responseXml, new TypeReference<Response<List<Schedule>>>() {
        });
        return response;
    }

    public Response<List<ClassTime>> findClassDoubleSchodule(ClassTime classTime) {
        String responseXml = POST(getSendUrl(ActionEnum.FINDCLASSDOUBLESCHODULE), classTime);
        Response<List<ClassTime>> response = JSON.parseObject(responseXml, new TypeReference<Response<List<ClassTime>>>() {
        });
    ...
    public Response<List<ClassTime>> findEmpDoubleSchodule(ClassTime classTime) {
        String responseXml = POST(getSendUrl(ActionEnum.FINDEMPDOUBLESCHODULE), classTime);
        Response<List<ClassTime>> response = JSON.parseObject(responseXml, new TypeReference<Response<List<ClassTime>>>() {
        });
